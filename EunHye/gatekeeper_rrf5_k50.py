...
# 역색인을 이용한 검색 (Metadata Boosting 적용 - Phase 1 최적화)
def sparse_retrieve(query_str, subject="", size=50):
    """
    BM25 검색에 Metadata Boosting을 적용 (Phase 1: size=50, boost=1.8)
    ...
    """
    if subject:
        # subject가 있으면 Metadata Boosting 적용 (부스팅 1.8로 하향 조정)
        query = {
            "bool": {
                "must": [
                    {"match": {"content": {"query": query_str, "boost": 1.0}}}
                ],
                # Phase 1: 부스팅을 1.8로 설정하여 오분류 리스크 감소
                "should": [
                    {"wildcard": {"src": {"value": f"*{subject}*", "boost": 1.8}}}
                ]
            }
        }
    else:
...
# RAG 구현에 필요한 질의 분석 및 검색을 위한 LLM 프롬프트 (Phase 1 수정: Gatekeeper 완화)
persona_function_calling = """
## Role: 지식 검색 전문가 및 Gatekeeper
...
# LLM과 검색엔진을 활용한 RAG 구현 (Phase 1: Gatekeeper + RRF 0.5:0.5 + 후보군 50)
def answer_question(messages):
    """
    Phase 1 최적화 RAG 시스템
    
    파이프라인:
    1. LLM으로 3개의 확장 쿼리 + 카테고리 생성 (Gatekeeper 포함)
    2. 각 쿼리별로 Sparse(Metadata Boosting 1.8) + Dense 검색 (각 50개)
    ...
    # [단계 1] 다중 쿼리 생성 및 카테고리 분류 (Gatekeeper 작동)
    ...
    try:
        result = client.chat.completions.create(
            ...
            messages=msg,
            tools=tools,
            tool_choice="auto",  # Phase 1: 일반 대화는 검색 건너뛰기 허용
            temperature=0,
            seed=1,
            timeout=10
        )
    ...
    if result.choices[0].message.tool_calls:
        ...
        # [단계 2] 각 확장 쿼리별로 검색 수행 및 RRF 통합 (Phase 1: size=50, RRF 0.5:0.5)
        for idx, q in enumerate(queries, 1):
            print(f"[DEBUG] Processing query {idx}/{len(queries)}: '{q}'")
            
            # Sparse 검색 (Metadata Boosting 1.8 적용, size=50)
            s_res = sparse_retrieve(q, subject, size=50)
            s_ranks = []
            ...
            # 개별 쿼리 내 RRF 결합 (Phase 1: 0.5:0.5 밸런스 가중치)
            s_rrf = calculate_rrf(s_ranks, k=60, weight=0.5)
            d_rrf = calculate_rrf(d_ranks, k=60, weight=0.5)
        ...
        # [단계 3] Reranking (Phase 1: 상위 50개 후보 대상)
        candidate_ids = sorted(all_query_rrf.items(), key=lambda x: x[1], reverse=True)[:50]
...
# Phase 1: Gatekeeper + RRF 0.5:0.5 + Boost 1.8 + 후보군 50 적용
eval_rag("../data/eval.jsonl", "phase1_v2_gatekeeper_relaxed_submission.csv")
