...
ES_PASSWORD = os.getenv("ES_PASSWORD", "-lDb+KIa69UGIdzacHAh")
...
mappings = {
    "properties": {
        ...
        # EN text (standard)
        "content_en": {"type": "text"},  # default standard analyzer
        "embeddings_en": {
            "type": "dense_vector",
            "dims": EMBED_DIM,
            "index": True,
...
# =========================================================
# 10) (NEW) Pair 생성 + Hard Negative + Dense Retriever Fine-tune
# =========================================================
def chunk_text(text: str, max_chars: int = 1200) -> str:
    """LLM 질문 생성용으로 너무 긴 문서를 앞부분만 잘라서 사용."""
    text = (text or "").strip()
...
def llm_generate_questions(client: OpenAI, passage: str, lang: str = "ko", n_q: int = 3) -> List[str]:
    """
    문서 passage에서 질문 n_q개 생성 (pseudo pair 생성)
    - lang: "ko" or "en"
    """
    passage = chunk_text(passage, max_chars=1400)

    if lang == "en":
        sys = "You generate short fact-seeking questions that can be answered from the given passage."
        user = f"""Passage:
{passage}

Task:
- Generate {n_q} questions that are directly answerable from the passage.
- Questions should be short and specific.
- Output only a JSON array of strings. No extra text."""
    else:
        sys = "너는 주어진 글에서 바로 답할 수 있는 짧고 구체적인 질문을 생성한다."
        user = f"""지문:
{passage}

요구사항:
- 지문만으로 답할 수 있는 질문 {n_q}개 생성
- 질문은 짧고 구체적으로
- 출력은 문자열 리스트(JSON 배열)만. 다른 텍스트 금지.
"""
    ...
    # JSON 파싱 실패 시: 라인 기반 fallback
    lines = [l.strip("-• \t") for l in txt.splitlines() if l.strip()]
    return lines[:n_q]


def build_doc_embeddings_local(model: SentenceTransformer, docs_joined: List[Dict[str, Any]], lang: str) -> np.ndarray:
    """
    Hard negative mining을 위해 로컬에서 문서 임베딩을 한 번 만들어둠.
    - lang: "ko" or "en"
    - 반환 shape: (N, D) normalized
    """
    texts = []
    key = "content_ko" if lang == "ko" else "content_en"
    for d in docs_joined:
        t = (d.get(key) or "").strip()
        texts.append(t if t else " ")

    vecs = model.encode(
        [e5_passage(t) for t in texts],
        normalize_embeddings=True,
...
def mine_hard_negative(
    ...
    """
    질문 q에 대해 doc_embs에서 cosine 상위 top_k 중
    pos_doc_idx가 아닌 문서 하나를 hard negative로 선택.
    """
    qv = model.encode(
        [e5_query(q)],
        normalize_embeddings=True,
    ...
    sims = doc_embs @ qv  # cosine (이미 normalized)
    # 상위 인덱스
    cand = np.argsort(-sims)[:top_k].tolist()
    for idx in cand:
        if idx != pos_doc_idx:
            return idx
    # fallback: 랜덤
    ridx = random.randrange(0, doc_embs.shape[0])
    return ridx if ridx != pos_doc_idx else (ridx + 1) % doc_embs.shape[0]


def finetune_dense_retriever_with_triplet(
    base_model_name_or_path: str,
    docs_joined: List[Dict[str, Any]],
    out_dir: str,
    lang: str = "ko",
    max_docs_for_qgen: int = 2000,
    q_per_doc: int = 2,
    hard_top_k: int = 30,
    epochs: int = 1,
    batch_size: int = 16,
    ...
    """
    TripletLoss로 (query, positive, hard_negative) 학습.
    - out_dir에 저장된 모델 경로를 반환.
    """
    random.seed(seed)
    np.random.seed(seed)
    ...
    # 2) (hard neg용) 문서 임베딩 사전 계산
    doc_embs = build_doc_embeddings_local(model, docs_joined, lang=lang)

    # 3) pseudo 질문 생성 + triplet 데이터 구성
    client = OpenAI(api_key=OPENAI_API_KEY)
    key = "content_ko" if lang == "ko" else "content_en"

    examples: List[InputExample] = []
    n_docs = min(len(docs_joined), max_docs_for_qgen)
    ...
    print(f"[FINETUNE] QGEN start | lang={lang} | docs_for_qgen={n_docs} | q_per_doc={q_per_doc} | hard_topk={hard_top_k}")

    for i in tqdm(range(n_docs), desc=f"QGEN({lang})", total=n_docs):
        passage = (docs_joined[i].get(key) or "").strip()
        if not passage:
            continue

        try:
            qs = llm_generate_questions(client, passage, lang=lang, n_q=q_per_doc)
        except Exception as e:
            fail_qgen += 1
            # 너무 많이 찍히면 지저분하니 가끔만
            if fail_qgen <= 3 or fail_qgen % 50 == 0:
                print(f"[QGEN][WARN] doc={i} qgen failed: {type(e).__name__}: {e}")
            continue
        ...
        for q in qs:
            total_q += 1
            if total_q % 200 == 0:
                print(f"[HN] mined {total_q} queries so far...")
            neg_idx = mine_hard_negative(model, q, pos_doc_idx=i, doc_embs=doc_embs, top_k=hard_top_k)
            pos_passage = passage
            neg_passage = (docs_joined[neg_idx].get(key) or "").strip() or " "
            examples.append(InputExample(texts=[e5_query(q), e5_passage(pos_passage), e5_passage(neg_passage)]))
        ...
        if now - last_log >= 10:
            elapsed = now - t0
            docs_done = i + 1
            rate_docs = docs_done / elapsed if elapsed > 0 else 0
            rate_q = total_q / elapsed if elapsed > 0 else 0

            print(
                f"[QGEN][STAT] docs={docs_done}/{n_docs} | "
                f"triplets={len(examples)} | q={total_q} | fail={fail_qgen} | "
                f"{rate_docs:.2f} doc/s | {rate_q:.2f} q/s | "
                f"elapsed={elapsed/60:.1f}m"
            )

            last_log = now

            # 60초마다 "저장했다는 로그만" 찍기 (파일 저장은 나중에)
            if (now - last_save) >= 60:
                print(f"[QGEN][SAVE] checkpoint | triplets={len(examples)} | elapsed={elapsed/60:.1f}m")
                last_save = now

    print(f"[FINETUNE] QGEN done | triplets={len(examples)} | total_q={total_q} | fail={fail_qgen} | elapsed={(time.time()-t0)/60:.1f}m")
    ...
    print(f"[FINETUNE] TRAIN start | triplets={len(examples)} | epochs={epochs} | bs={batch_size} | lr={lr} | warmup={warmup_steps}")
    t_train = time.time()
    ...
    print(f"[FINETUNE] TRAIN done | elapsed={(time.time()-t_train)/60:.1f}m | saved={out_dir}")
...
if __name__ == "__main__":
    ...
    # (NEW) Dense retriever finetune 옵션
    TRAIN_DENSE = os.getenv("TRAIN_DENSE", "false").lower() == "true"
    FINETUNE_LANG = os.getenv("FINETUNE_LANG", "ko")  # "ko" or "en"
    FINETUNE_OUT_DIR = os.getenv("FINETUNE_OUT_DIR", "./finetuned_dense")
    Q_PER_DOC = int(os.getenv("Q_PER_DOC", "2"))
    MAX_DOCS_FOR_QGEN = int(os.getenv("MAX_DOCS_FOR_QGEN", "1500"))
    HARD_TOPK = int(os.getenv("HARD_TOPK", "30"))
    FT_EPOCHS = int(os.getenv("FT_EPOCHS", "1"))
    FT_BS = int(os.getenv("FT_BS", "16"))
    FT_LR = float(os.getenv("FT_LR", "2e-5"))
    ...
    # (NEW) 문서 기반 pseudo pair로 dense retriever finetune (선택)
    if TRAIN_DENSE:
        # base 모델은 현재 EMBED_MODEL_NAME(=embed_model이 로드한 모델)
        finetuned_path = finetune_dense_retriever_with_triplet(
            base_model_name_or_path=EMBED_MODEL_NAME,
            docs_joined=docs_joined,
            out_dir=FINETUNE_OUT_DIR,
            lang=FINETUNE_LANG,
            max_docs_for_qgen=MAX_DOCS_FOR_QGEN,
            q_per_doc=Q_PER_DOC,
            hard_top_k=HARD_TOPK,
            epochs=FT_EPOCHS,
            batch_size=FT_BS,
            lr=FT_LR,
            seed=SEED,
        ...
        # ⚠️ 중요: finetune된 모델로 embed_model 교체 + 차원도 갱신
        embed_model = SentenceTransformer(finetuned_path)
        EMBED_DIM = int(embed_model.get_sentence_embedding_dimension())

        # dims가 바뀌면 ES mapping과 충돌하므로 mappings도 갱신 필요
        mappings["properties"]["embeddings_ko"]["dims"] = EMBED_DIM
        mappings["properties"]["embeddings_en"]["dims"] = EMBED_DIM

        # 인덱스 재생성(차원 변경 가능성 대비)
        create_es_index(ES_INDEX, settings, mappings)
        print(f"[FINETUNE] Reloaded embed_model from {finetuned_path}, EMBED_DIM={EMBED_DIM}")
